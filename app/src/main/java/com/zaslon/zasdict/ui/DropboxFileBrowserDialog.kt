...
/**
 * Dropbox のフォルダを階層ブラウズしてファイルを選択するダイアログ。
 * 状態（entries / loading / error）は ViewModel 側で管理する。
 */
@Composable
fun DropboxFileBrowserDialog(
    entries: List<DropboxApiClient.FileEntry>,
    currentPath: String,
    isLoading: Boolean,
    error: String?,
    ...
    val title = if (currentPath.isEmpty()) "Dropbox" else currentPath.substringAfterLast("/")
